 ...
 * del driver). Se aplica el SQL de la migración generada por drizzle-kit
 * directamente, sin depender del cargador de .sql de Metro (solo existe en RN).
 */
export function crearBaseDeDatosDePrueba(): AppDatabase {
  const sqlite = new Database(':memory:');
  const rutaMigracion = path.resolve(
    __dirname,
    '../../drizzle/migrations/0000_sharp_jean_grey.sql',
  );
  const sql = fs.readFileSync(rutaMigracion, 'utf-8');
